class ValidationPatterns {
  ...
  // Mobile Number
  static final RegExp mobileNumber = RegExp(r'^(?:\+91)?[1-9][0-9]{9}$');
  ...
  // Date of Birth
  static final RegExp dateOfBirth =
  ...
  // Website
  static final RegExp website = RegExp( r'^(https?:\/\/)?'); // Match http:// or https:// (optional))

  // Lead Priority
  static final RegExp leadPriority = RegExp(r'^(High|Medium|Low)$');

  // Lead Source
  static final RegExp leadSource =
      RegExp(r'^(LinkedIn|Website|Referral|Other)$');

  // Lead Stages
  static final RegExp leadStages =
      RegExp(r'^(New Lead|Contacted|Converted|Lost)$');

  // Aadhar Number
  static final RegExp aadharNumber =
  ...
  // GST Number
  static final RegExp gstNumber = RegExp(
      r'^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[A-Z]{1}[0-9A-Z]{1}$');

  // GST State
  static final RegExp gstState = RegExp(
      r'^(Maharashtra|Karnataka|Tamil Nadu|...)$'); // Replace ... with your list
  ...
  // Google Location
  static final RegExp googleLocation =
      RegExp(r'^https:\/\/(www\.)?maps\.google\.com\/.*$');
  ...
  // Attachments
  static final RegExp attachments =
      RegExp(r'^[A-Za-z0-9-_\s]+\.(pdf|jpg|jpeg|png|doc|docx)$');
}
